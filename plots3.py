...
def make_dBZ_hist(batch, dBZ_range=(-27,20.1,1)):
...
def generated_hist(gen, modis_vars, modis_mask, batch_size=128,
    noise_dim=64):
...
def plot_hist(hist_real, hist_gen, pix_extent=(0.24,1.09)):
    ...
    norm = colors.Normalize(0,0.0015)
    norm_diff = colors.Normalize(-0.0005,0.0005)
    # norm_diff = colors.Normalize(-max_diff,max_diff)

    ax = plt.subplot(gs[0,0])
    plt.imshow(hist_real_norm, aspect='auto', norm=norm,
        extent=[-27,20,0,scene_size*pix_extent[0]],
        cmap=vir_white)
    plt.xlabel("Reflectivity [dBZ]")
    plt.ylabel("Altitude [km]")
    ax.set_xticks([-20,-10,0,10,20])
    ax.set_yticks([0,4,8,12])
    cb = plt.colorbar(cm.ScalarMappable(norm=norm),orientation='horizontal')
    cb.ax.set_xlabel("Norm. occurrence (real)")
    cb.ax.tick_params(labelsize=8,labelrotation=30)
...
def load_data_and_models(scenes_fn, model_name="cs_modis_cgan", 
...
def plot_all(scenes_fn, model_name="cs_modis_cgan"):
    ...
    plt.savefig("../figures/real_gen_hist.svg", dpi=1200,bbox_inches='tight', format='svg')
    plt.close()
