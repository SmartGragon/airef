...
def plot_samples_cmp(gen, scene_real,labels_real, modis_vars, modis_mask, noise_dim=64,
    ...
    for i in range(num_samples):
        ...
        for k in range(num_gen):
            ...
            if (k!=num_gen-1):
                ...
                plot_label(ax_gen, labels_real[i,:,:,0], pix_extent, scene_size)
    ...
    add_dBZ_colorbar(fig, [0.91, 0.365, 0.018, 0.38])
    add_label_colorbar(fig, [0.91, 0.115, 0.018, 0.24])
    return fig
...
def plot_samples_cmp_all(gen, scene_real, modis_vars, modis_mask,labels_real):
    ...
    samples_sel_1 = [855, 116,821, 307, 641, 663, 64, 746]
    samples_sel_2 = [547, 748, 190, 750,306, 207, 438, 776]
...
def generated_hist(gen, modis_vars, modis_mask,scene_real, scenes_fn,
    batch_size=16, noise_dim=64):
    ...
    for i in range(0,N,batch_size):
        ...
        df.loc[len(df)] = ['15dBZ',hss,'HSS']
        # df.loc[len(df)] = ['15dBZ',bss,'BSS']
        # rmse = accloss.calculatermse(scene_real_batch,gen_batch)
    ...
    plt.legend(title='Threshold',frameon=True,facecolor='#f0f8ff')
    ax=plt.gca()
    ax.set_ylabel("Score", fontsize=26)
    ax.set_xlabel("Class",fontsize=26)
    # Rank = rank.item()
    # plt.text(0.05, 0.95,f"RMSE= {Rank:.2f}dBZ", color='red',fontsize=15, transform=ax.transAxes)
    gen_batch = data_utils.rescale_scene_gen(gen_batch)
    plt.ylim([-0.05,1.05])
    plt.savefig("../data/naccuracy.svg", dpi=1200,bbox_inches='tight', format='svg')
...
def plot_all(scenes_fn, model_name=pick_model):
    ...
    hist_gen = generated_hist(gen, modis_vars, modis_mask,scene_real,scenes_fn)
