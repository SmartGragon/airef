...
def load_cloudsat_labels(fn, n=None, right_handed=False, frac_validate=0.1,
    shuffle=True, shuffle_seed=None):

    with netCDF4.Dataset(fn, 'r') as ds:
        if n is None:
            n = ds["label"].shape[0]
        cs_scenes = np.array(ds["label"][:n,:,:])
        cs_scenes = cs_scenes.reshape(cs_scenes.shape+(1,))
        if right_handed:
            cs_scenes = np.rot90(cs_scenes, axes=(2,1))
        # rescale from (0,1) to (-1,1)
...
def rescale_scene_gen(scene, Z_range=(-35,20), missing_max=-27):
    sc = Z_range[0] + (scene+1)/2.0 * (Z_range[1]-Z_range[0])
    sc[sc <= missing_max] = np.nan
    return sc

def rescale_scene(scene, Z_range=(-32,20), missing_max=-27):
    sc = Z_range[0] + (scene+1)/2.0 * (Z_range[1]-Z_range[0])
    sc[sc <= missing_max] = np.nan
    return sc
